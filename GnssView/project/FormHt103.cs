        ...
        public void ht103CmdDec(byte[] data, int len)
        {
            ...
            try
            {
                this.Invoke(new MethodInvoker(delegate
                {
                    ...
                    label26.Text = BitConverter.ToInt16(data, 11).ToString();
                    label4.Text = data[13].ToString();
                    label8.Text = data[14].ToString();
                    label12.Text = data[15].ToString();
                    label16.Text = (BitConverter.ToInt32(data, 16) / 1000.0).ToString("f3");
                    label20.Text = data[20].ToString();
                    label24.Text = data[21].ToString();
                    label28.Text = BitConverter.ToInt16(data, 22).ToString();
                    label30.Text = data[24].ToString();
                    label32.Text = data[25].ToString();
                    label34.Text = data[26].ToString();
                    label36.Text = data[27].ToString();
                    label38.Text = (BitConverter.ToInt32(data, 28) * 1e-7).ToString("f8");
                    label40.Text = (BitConverter.ToInt32(data, 32) * 1e-7).ToString("f8");
                    label42.Text = (BitConverter.ToInt32(data, 36) * 1e-7).ToString("f8");
                    label44.Text = (BitConverter.ToInt32(data, 40) * 1e-7).ToString("f8");
                    label46.Text = (BitConverter.ToInt32(data, 44) * 1e-7).ToString("f8");
                    label48.Text = (BitConverter.ToInt32(data, 48) * 1e-7).ToString("f8");
                    label50.Text = (BitConverter.ToInt16(data, 52) * 0.01).ToString("f8");
                    label52.Text = (BitConverter.ToInt16(data, 54) * 0.01).ToString("f8");
                    label54.Text = (BitConverter.ToInt16(data, 56) * 0.01).ToString("f8");
