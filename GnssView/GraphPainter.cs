...
namespace GnssView
{
    public class GraphPainter
    {
        ...
        public void InitGraphPositions(Graphics g, Size clientSize, int startX, int endCnt)
        {
            CalculateCoordinateInfo(ref clientSize, startX, endCnt);
        }
        ...
        private void CalculateCoordinateInfo(ref Size boardSize, int startX, int endCnt)
        {
        ...
        private void DrawXAxesDataTypeText(Graphics g, Brush b, string text, float rightPointX, float rightPointY, float height)
        {
            float textWidth = g.MeasureString(text, CoordinateInfo.AxesFont, Convert.ToInt32(height)).Width;
            RectangleF rectf = new RectangleF(rightPointX + 2 * textWidth, rightPointY - height - 2 * CoordinateInfo.AxesFont.Height, textWidth, height);
            g.DrawString(text, CoordinateInfo.AxesFont, b, rectf);
        }

        public void DrawCoordinate(Graphics g, Size boardSize, int startX, int endCnt)
        {
            ...
            try
            {
                ...
                tmpPoint.X = 0;
                baseValue = startX;//0;
                stepValue = endCnt / CoordinateInfo.AXES_TOTAL_MILLIMETRE;//CoordinateInfo.AXES_TOTAL_MILLIMETRE*1.0f / CoordinateInfo.AXES_BIG_BLOCK_COUNT;
                ...
                g.DrawLine(Pens.Black, tmpPoint.X, tmpPoint.Y, tmpPoint.X, tmpPoint.Y + CoordinateInfo.AXES_X_LONG_GRADUATION_HEIGHT);
                DrawXAxesLongGraduationText(g, Brushes.Black, Convert.ToString(baseValue + stepValue * CoordinateInfo.AXES_BIG_BLOCK_COUNT), tmpPoint.X, tmpPoint.Y + CoordinateInfo.AXES_X_LONG_GRADUATION_HEIGHT);
                //绘制最上面一条长刻度线上的数据类型的文本
                //DrawXAxesDataTypeText(g, sb, "X", tmpPoint.X, tmpPointY, CoordinateInfo.AXES_X_LONG_GRADUATION_HEIGHT * 2);
                #endregion
            }
            catch (Exception ex)
            {
